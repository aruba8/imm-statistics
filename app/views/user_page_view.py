...
STREAM_CHOICES = ['general', 'family', 'strategic']
...
def user_edit_page():
    from app.models.userdata import UserData
    userdata = UserData.objects(username=current_user.username).first()
    form = UserDataForm(interview_location=userdata.interview_location,
    ...
    form.stream.choices = [(value, value) for value in generate_choices_streams(userdata.stream, STREAM_CHOICES)]

    if request.method == 'POST':
        UserData.objects(username=current_user.username).update(set__from_full=form.from_full.data,
        ...
        return redirect(url_for('user.user_edit_page'))
    return render_template('user_page_edit.html', userdata=userdata, form=form)


def generate_choices_streams(first_choice, choices):
    if first_choice is not None:
        choices_gen = [first_choice]
        for choice in choices:
            if choice != first_choice:
                choices_gen.append(choice)
        return choices_gen
    else:
        return choices